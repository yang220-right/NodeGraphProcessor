...
namespace GraphProcessor{
  ...
  public class BaseGraphView : GraphView, IDisposable{
    ...
    // 来自BaseGraph的安全事件中继（安全是因为当调用这些事件之一时，您确定始终指向有效的BaseGraph），
    // 不过图形切换可能发生在两次调用之间
    /// <summary>
    /// 与BaseGraph.onExposedParameterListChanged相同的事件
    /// 安全事件（在图形为null的情况下不会触发）。
    /// </summary>
    public event Action onExposedParameterListChanged;

    /// <summary>
    /// 与BaseGraph.onExposedParameterModified相同的事件
    /// 安全事件（在图形为null的情况下不会触发）。
    /// </summary>
    public event Action<ExposedParameter> onExposedParameterModified;

    /// <summary>
    /// 当节点被复制（ctrl-d）或复制粘贴（ctrl-c/ctrl-v）时触发
    /// </summary>
    ...
    protected virtual void BuildGroupContextualMenu(ContextualMenuPopulateEvent evt, int menuPosition = -1){
      ...
      evt.menu.InsertAction(menuPosition, "Create Group",
        (e) => AddSelectionsToGroup(AddGroup(new Group("Create Group", position))), DropdownMenuAction.AlwaysEnabled);
    }
    ...
    void InitializeEdgeViews(){
      ...
      foreach (var serializedEdge in graph.edges){
        ...
        edgeView.input =
          inputNodeView.GetPortViewFromFieldName(serializedEdge.inputFieldName, serializedEdge.inputPortIdentifier);
        edgeView.output =
          outputNodeView.GetPortViewFromFieldName(serializedEdge.outputFieldName, serializedEdge.outputPortIdentifier);
    ...
    /// </summary>
    public void DelayedDeleteSelection() => this.schedule
      .Execute(() => DeleteSelectionOperation("Delete", AskUser.DontAskUser)).ExecuteLater(0);
